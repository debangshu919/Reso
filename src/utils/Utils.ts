...
import type { Context, Reso } from "../classes/index"
...
export class Utils {
	...
	public static getNextStatus(): { text: string; type: number } {
		const status = Utils.statusMessages[Utils.currentStatusIndex]
		Utils.currentStatusIndex =
			(Utils.currentStatusIndex + 1) % Utils.statusMessages.length
		return status
	}
	...
	public static updateStatus(client: Reso, guildId?: string): void {
		...
		if (user) {
			if (client.env.GUILD_ID && guildId === client.env.GUILD_ID) {
				...
				const status = Utils.getNextStatus()
				user.setPresence({
					activities: [
						{
							name: status.text,
	...
	public static async paginate(
		...
		const filter = (int: Interaction) => int.user.id === author?.id
